...
*   ---------------------------------------------------------------------------
*/
int save_preset(string const name, IO_Potentiometer pot[SPI_POTAR_COUNT]){
    ...
    try{
        ...
        if(flux){
            ...
            for ( Module_iterator itr = Module_Node_List::Get().list_.begin() ; itr != Module_Node_List::Get().list_.end(); itr++ ){
            ...
            Connection_List cl = Module_Node_List::Get().connection_get_list();
                    ...
                    // If waiting node begin carracter
                        ...
                        if ( is_file_ok && is_version_ok ){
                            if ( !clear_once ){
                                ...
                                Module_Node_List::Get().clear_graph();
                                clear_once = true;
                            }
                        }
                        else{
                    ...
                    // Module node
                        ...
                        if ( !string(token[i]).compare(NMOD_TYPE) ){
                            ...
                            if ( iok ){
                                ...
                                if ( Module_Node_List::Get().add_module( cur_type, cur_id ) ){

                                    flag = FROOT;
                                    cout << "Error : Invalid Module Type" << endl;
                                }
                                cur_mod = Module_Node_List::Get().list_.size() -1;
                            }
                        }
                        // Module's ID
                        ...
                        //Exit Datas
                            ...
                            if ( dat_idx == dat_size ){
                                ...
                                if ( cur_bak == 0 ){
                                    ...
                                    Module_Node_List::Get().list_[cur_mod]->set_param( dat_size, dat_buff );
                                }
                                else{

                                    Module_Node_List::Get().list_[cur_mod]->add_bank( dat_size, dat_buff );
                                }
                            }
                            else{
                    ...
                    // Connection node
                        ...
                        if ( !!string(token[i]).compare("}") ){
                            ...
                            if ( con_idx == 4 ){
                                ...
                                Module_Node_List::Get().add_connection( con_buff[0], con_buff[1], con_buff[2], con_buff[3] );
                            }
                            else{
...
int list_files(string const dir, vector<string> & list, bool log){

    if ( log ) cout << "List files in Directory : \"" << dir << "\"" << endl;
    ...
    if ( rep != NULL ){
        ...
        while ( (ent = readdir( rep )) != NULL ){
            ...
            if ( ent->d_name[0] != '.') {
                ...
                if ( log ) cout << "    File : \"" << ent->d_name << "\"" << endl;
        ...
        if ( log ) cout << "    Error : Cant't Open Given Dir" << endl;
        return 1;
    }
    ...
    if ( list.size() == 0 ){
        ...
        if ( log ) cout << "    Error : No Files Founded" << endl;
        return 1;
    }
    return 0;
}
