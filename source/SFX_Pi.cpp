...
int main(int argc, char *argv[]){
    ...
    if ( io_init_frontPanel() ){

        cout << "Error Loading Front Panel, Abord Program" << endl;
        exit(1);
    }
    ...
    */
    while(1){
        ...
        */
        if ( static_cast<EndModule*>(Module_Node_List::Get().begin_)->is_saturated() ){

            IOS::printm( "Wrn Input", IOS::DEFAULT_TEMP , IOS::TEMP);
            static_cast<EndModule*>(Module_Node_List::Get().begin_)->reset_saturated();
        }

        /*
        * Check if output is saturated
        */
        if ( static_cast<EndModule*>(Module_Node_List::Get().end_)->is_saturated() ){

            IOS::printm( "Wrn Output", IOS::DEFAULT_TEMP , IOS::TEMP );
            static_cast<EndModule*>(Module_Node_List::Get().end_)->reset_saturated();
        }
