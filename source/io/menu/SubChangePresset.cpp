 ...
 */
bool func_reload_presset_list( bool log = false );

/**
 * Function called for load current presset
 */
void func_load_preset( IO_Potentiometer[SPI_POTAR_COUNT] );
...
MenuIterator main_change_presset_do( Move_flag action, MenuIterator itr, IO_Potentiometer pot[SPI_POTAR_COUNT] ){
    ...
    if ( action == MOVE_UP ){
        ...
        if ( presset_list.size() != 0 ){
            ...
            if ( save_preset( (*current_presset), pot) ){
...
void func_load_preset( IO_Potentiometer pot[SPI_POTAR_COUNT] ){

    if ( load_preset( (*current_presset), pot) ){
