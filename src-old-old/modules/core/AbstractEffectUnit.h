...
class AbstractEffectUnit{
    ...
    public :
         ...
         * @see JACKUnit.h
         **/
        AbstractEffectUnit(id1_t id, id1_t type, usize_t bankSize, usize_t paramSize);
         ...
         * bank size
         **/
        void setParam(usize_t idx, float value);
        void setParam(usize_t size, const float *values);
        float getParam(usize_t idx)  const;
         ...
         **/
        virtual JACKUnit* getJACKUnit(usize_t pidx);
...
namespace EUCST{
    ...
    const usize_t C_PARAM_COUNT = 2;
    
    const usize_t IDXBYP  = 0;
    const usize_t IDXVOL  = 1;

    const std::string C_NAME[C_PARAM_COUNT] = {"Bypass", "Volume"};

    const std::string JACK_SERVER = SFXP::JACK_SERVER;
