...
class ParametricEQ : public AFilterBase{
    ...
    public :
        ...
        ParametricEQ(usize_t poleCount, float* poles, float* qf, float samplerate);
         ...
         **/
        sample_t compute(sample_t in, usize_t poleCount, float* gains);
        sample_t compute(sample_t in, usize_t idx);
        sample_t getBand(usize_t idx);

        void setPole(usize_t idx, float f, float q, float sr);

        /**
         * Change EQ Configuration
         **/
        void setFrequency(usize_t idx, float f, float sr);
        void setFrequency(usize_t poleCount, float* poles, float sr);
        float getFrequency(usize_t idx) const;
        
        /**
         * Change EQ Configuration
         **/
        void setQFactor(usize_t idx, float qf, float sr);
        void setQFactor(usize_t poleCount, float* qf, float sr);
        float getQFactor(usize_t idx) const;

        usize_t getBandCount() const;
    ...
    private :
         ...
         **/
        void updatePole(usize_t idx, float cf, float qf, float sr);
