...
namespace{
    ...
    const usize_t BIN = 1 << (SFXP::JACK_BLOCK_SIZE-1);
}
/* ***************************** Consts ***************************** */
...
template<> const usize_t FFTFilterReg::PARCOUNT = 3;

template<> const std::string FFTFilterReg::PORNAMES[] = {
    "Input", "Output"};

template<> const usize_t FFTFilterReg::AI = 1;
template<> const usize_t FFTFilterReg::AO = 1;
template<> const usize_t FFTFilterReg::MI = 0;
template<> const usize_t FFTFilterReg::MO = 0;

template<> const usize_t FFTFilterReg::PARSIZE = BIN;
/* ************************* FFTComplexifier ************************ */
/**
...
void FFTFilter::update(){
    ...
    // If filter is bandpass
        ...
        for ( usize_t i = 0; i < m_paramSize-OFFP; i++ ){
