...
template<> const usize_t LFOReg::PARCOUNT = 8;

template<> const std::string LFOReg::PORNAMES[] = {"Sync","Output","MIDI-In"};

template<> const usize_t LFOReg::AI = 1;
template<> const usize_t LFOReg::AO = 1;
template<> const usize_t LFOReg::MI = 1;
template<> const usize_t LFOReg::MO = 0;

template<> const usize_t LFOReg::PARSIZE = LFOReg::PARCOUNT +2;
    ...
    m_wave(castWaveform(WSIN))
{
    ...
    try{
        ...
        m_jackU->registerPorts( PORNAMES, AI, AO, MI, MO );
...
int LFOEffect::process(jack_nframes_t nframes, void* arg){
    ...
    if ( !unit->m_paramArray[EUCST::IDXBYP] ){
        ...
        void* midi_port = jack_port_get_buffer( unit->m_jackU->getPorts()[2], nframes);
        jack_midi_event_t in_event;
        jack_nframes_t event_index = 0;
        jack_nframes_t event_count = jack_midi_get_event_count(midi_port);

        // Negative frequency is used to set lfo midi responsive
        if ( p[FRQ] < 0 ){

            if ( event_count > 1 ){

                if ( SFXP::GlobalIsDebugEnabled)
                    printf("%d events availables\n", event_count);
                    
                for ( jack_nframes_t i = 0; i < event_count; i++ ){

                    jack_midi_event_get(&in_event, midi_port, i);
                    if ( SFXP::GlobalIsDebugEnabled)
                        printf("   event %d time is %d, first byte is 0x%x\n", i, in_event.time, in_event.buffer[0]);
                }
            }
            ...
            if ( p[FRQ] < 0 ){
                ...
                if ( in_event.time == i && event_index < event_count ){
                    ...
                    if ( (in_event.buffer[0] & 0xf0) == 0x90 ){
                        ...
                        p[PRBS] = MIDIBase::getNoteFrequency(in_event.buffer[1]);
                        p[EUCST::IDXVOL] = 1.0f;
                        
                        if ( SFXP::GlobalIsDebugEnabled)
                            printf("Midi note on : %x : Note : %i : Velocity : %i : Rbs : %f\n", in_event.buffer[0], in_event.buffer[1], in_event.buffer[2], p[PRBS]);
                    }
