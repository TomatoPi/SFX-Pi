...
template<> const usize_t DriveReg::PARCOUNT = 19;

template<> const std::string DriveReg::PORNAMES[] = {"Input", "Output"};

template<> const usize_t DriveReg::AI = 1;
template<> const usize_t DriveReg::AO = 1;
template<> const usize_t DriveReg::MI = 0;
template<> const usize_t DriveReg::MO = 0;

template<> const usize_t DriveReg::PARSIZE = DriveReg::PARCOUNT;
...
int DriveEffect::process(jack_nframes_t nframes, void* arg){
    ...
    if ( !unit->m_paramArray[EUCST::IDXBYP] ){
        ...
        for ( jack_nframes_t i = 0; i < nframes; i++ ){
            ...
            out[i] = p[EUCST::IDXVOL] * unit->m_toneOut->compute( out[i], 3, p + OLG );
        }
    }
    else{
