...
template<> const usize_t MDriveReg::PARCOUNT = 36;
...
template<> const usize_t MDriveReg::AI = 1;
template<> const usize_t MDriveReg::AO = 6;
template<> const usize_t MDriveReg::MI = 0;
template<> const usize_t MDriveReg::MO = 0;

template<> const usize_t MDriveReg::PARSIZE = MDriveReg::PARCOUNT;
...
int MatrixDriveEffect::process(jack_nframes_t nframes, void* arg){
    ...
    const usize_t bandCount = (usize_t)unit->m_paramArray[BC];
...
void MatrixDriveEffect::update(){
    ...
    for ( usize_t i = 0; i < (usize_t)m_paramArray[BC]; i++ ){
