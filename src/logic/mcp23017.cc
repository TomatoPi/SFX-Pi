...
#define NAME ("MCP:"+std::to_string(this->deviceAddress))
...
mcp23017::mcp23017(void){
	...
	SFXPlog::log(NAME) << "Opening I2C Device" << endl;
    this->openI2C();
}
 ...
 * function call.
 * *****************************************************************/
mcp23017::mcp23017(hex_t dev_addr, std::string i2c_file_name){
	
    this->i2cFileName = i2c_file_name;
	this->deviceAddress = dev_addr;
	this->i2cDescriptor = -1;
    SFXPlog::log(NAME) << "Open I2C Device : " << i2c_file_name << endl;
    this->openI2C();
}
...
mcp23017::~mcp23017(void){
    ...
    SFXPlog::log(NAME) << "Closing I2C Device" << endl;
    this->closeI2C();
}
...
int mcp23017::openI2C(){
    ...
    if(this->i2cDescriptor < 0){
        SFXPlog::err(NAME) << "Could not open file (1)" << endl;
        return 1;
    }
...
int mcp23017::closeI2C(){
    ...
    if(retVal < 0){
        SFXPlog::err(NAME) << "Could not close file (1)" << endl;
        return 1;
    }
	return retVal;
}
 ...
 ********************************************************************/
int mcp23017::writeReg(hex_t reg_addr, hex_t data){
    ...
    if(retVal < 0)
        SFXPlog::err(NAME) << "Write to I2C Device failed" << endl;

    return retVal;
}

int mcp23017::writeReg(hex_t reg_addr, hex_t mask, hex_t data){
 ...
 *variable "data".
 ********************************************************************/
int mcp23017::readReg(hex_t reg_addr, hex_t &data){

    hex_t *inbuff, outbuff;
    int retVal = -1;
    struct i2c_rdwr_ioctl_data packets;
    ...
    retVal = ioctl(this->i2cDescriptor, I2C_RDWR, &packets);
    if(retVal < 0)
        SFXPlog::err(NAME) << "Read from I2C Device failed" << endl;
