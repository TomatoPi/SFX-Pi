...
int LogicManager::create(){
    ...
    for ( size_t i = 0; i < MCP_COUNT; i++ ){
        ...
        m_MCPReg[i]->writeReg(mcp23017::HEX_IODIRA, m_mask_IODIR[i][0]);
        m_MCPReg[i]->writeReg(mcp23017::HEX_IODIRB, m_mask_IODIR[i][1]);

        m_MCPReg[i]->writeReg(mcp23017::HEX_IPOLA, m_mask_IPOL[i][0]);
        m_MCPReg[i]->writeReg(mcp23017::HEX_IPOLB, m_mask_IPOL[i][1]);
    }
...
LogicManager::LEDReg::LEDReg(LED *led, size_t mcp, bool gpb, hex_t adr):
    m_led(led),
    m_mcp(mcp),
    m_gpiob(gpb),
    m_adr(adr)
