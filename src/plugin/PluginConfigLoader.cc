...
bool loadConfig(Plugin* plugin, string path){
    ...
    if (config.LoadFile((path+ConfigFile).c_str())) {
        ...
        cout << "PluginConfigLoader : Error : #" << config.ErrorID() << " : " <<
    ...
    while (node != NULL){
        ...
        if (elem != NULL) {
            ...
            if (string(elem->Name()) == "port") {
                ...
                pair<PortType,string> port = parsePortNode(elem);
 ...
 **/
pair<PortType,string> parsePortNode(XMLElement* port){
    ...
    if (t == "AudioIn") type = PortType::AudioIn;
    else if (t == "AudioOut") type = PortType::AudioOut;
    else if (t == "MidiIn")   type = PortType::MidiIn;
    else if (t == "MidiOut")  type = PortType::MidiOut;
    else success = false;
