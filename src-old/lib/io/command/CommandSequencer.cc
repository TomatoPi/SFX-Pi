 ...
 **/
CommandSequencer::CommandSequence CommandSequencer::pushNotification(Notification notif){
    ...
    if ( m_actions.find(notif) != m_actions.end() ){
        ...
        return this->performAction(m_actions[notif]);
    }
    else{
 ...
 **/
CommandSequencer::CommandSequence CommandSequencer::performAction(Action act){
