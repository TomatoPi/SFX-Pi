...
int CommandManager::create(){
    ...
    printf("Command Manager : Start Manager\n");
...
void CommandManager::pushNotification(Notification notif){
    ...
    if ( m_seqList.find(notif.target()) != m_seqList.end() ){
        ...
        CommandSequencer::CommandSequence sequence = m_seqList[notif.target()]->pushNotification(notif);

        for (auto & cmd : sequence) {

            CommandFactory::performCommand(
