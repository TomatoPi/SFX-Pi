    ...
    try{
        ...
        for ( usize_t i = 0; i < MCP_COUNT; i++ ){
            ...
            if ( m_MCPReg[i]->writeReg(mcp23017::HEX_IODIRA, m_mask_IODIR[i][0]) < 0 ){
                ...
                throw std::string("Failed Write IODIRA on MCP_%u", i);
            }
                
            if ( m_MCPReg[i]->writeReg(mcp23017::HEX_IODIRB, m_mask_IODIR[i][1]) < 0 ){

                throw std::string("Failed Write IODIRB on MCP_%u", i);
            }
                
            if ( m_MCPReg[i]->writeReg(mcp23017::HEX_IPOLA, m_mask_IPOL[i][0]) < 0 ){

                throw std::string("Failed Write IPOLA on MCP_%u", i);
            }
                
            if ( m_MCPReg[i]->writeReg(mcp23017::HEX_IPOLB, m_mask_IPOL[i][1]) < 0 ){

                throw std::string("Failed Write IPOLB on MCP_%u", i);
            }
        }
...
LogicManager::LEDReg::LEDReg(LED *led, usize_t mcp, bool gpb, hex_t adr):
    m_led(led),
    m_mcp(mcp),
    m_gpiob(gpb),
    m_adr(adr)
