...
Plugin* loadPlugin(string path){
    ...
    try{
        ...
        // Try Load his config
        if (PluginConfigLoader::loadConfig(plugin, path)) {
        ...
        if (plugin->status()) {
            ...
            throw string("Not Catched Internal Load Error");
        }
    }
    catch (string & e) {
...
bool EffectFactory::buildRegistry(){
    ...
    if (TCTLoader::loadTypeCodeTable()) return true;
    ...
    for (auto & dir : TCTLoader::typeCodeTable()) {
        ...
        Plugin* plugin = PluginLoader::loadPlugin(SFXP::DIR_PLUG+dir.second);
